...
## Target zoom level to use during the test
@export var test_zoom_level: Vector2 = Vector2(1.5, 1.5)
...
## Shake strength to apply during test (0.0 to 1.0)
@export_range(0.0, 1.0) var test_shake_strength: float = 0.8

# --- TEST INPUT KEYS ---
@export_group("Testing Keys")
## Enable key listening for tests during gameplay
@export var enable_keyboard_testing: bool = true
@export var test_zoom_key: Key = KEY_Z
@export var test_shake_key: Key = KEY_X
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo:
		if event.keycode == test_zoom_key:
			test_zoom_toggle()
		elif event.keycode == test_shake_key:
			test_shake()
...
# --- INSPECTOR & KEYBOARD TEST FUNCTIONS ---

## Toggles between default zoom and test_zoom_level
func test_zoom_toggle() -> void:
	if is_zoomed_in:
		zoom_camera(default_zoom)
		is_zoomed_in = false
	else:
		zoom_camera(test_zoom_level)
		is_zoomed_in = true

## Triggers a test shake using test_shake_strength parameter
func test_shake() -> void:
	shake_camera(test_shake_strength)
